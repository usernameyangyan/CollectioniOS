...
- 如果不实现YYIBaseTableViewController，可通过：
...
###### 注意：YYHttpRequestUtils在请求时候需要传入一个泛型类作为json数据的转换数据结构，bean类需要实现Convertible才能转化
...
| removeAllCache(completion: @escaping (Bool)->())| 清除所有缓存 | 
...
|cacheTimeOutWithClear(expiry: YYHttpExpiry)| 缓存过期时间|
