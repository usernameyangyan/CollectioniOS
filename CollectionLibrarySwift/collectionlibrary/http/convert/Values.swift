...
public enum Values {
    static func value(_ val: Any?,
        ...
        guard let v = val.yy_value else { return nil }
        ...
        case let enumType as ConvertibleEnum.Type:
            let vv = value(v, enumType.yy_valueType)
            return enumType.yy_convert(from: vv as Any)
        default: return nil
        }
    }
    
    static func JSONValue(_ value: Any?) -> Any? {
        guard let v = value.yy_value else { return nil }
        if v is NSNull { return nil }
        
        switch v {
        case let num as NumberValue: return _JSONValue(from: num)
        case let model as Convertible: return model.yy_JSONObject()
        ...
        case let `enum` as ConvertibleEnum: return JSONValue(`enum`.yy_value)
    ...
    static func JSONString(_ value: Any?,
        ...
        if let str = JSONSerialization.yy_string(JSONValue(value),
...
private extension Values {
    ...
    static func _model(_ value: Any,
        ...
        if var model = defaultValue().yy_value as? Convertible {
            model.yy_convert(from: json)
            return model
        }
    ...
    static func _array(_ value: Any, _ type: Any.Type) -> ArrayValue? {
        ...
        if let type = (mt as? NominalType)?.genericTypes?.first {
            if let modelType = type~! as? Convertible.Type {
                return json.yy.modelArray(type: modelType)
            } else if let enumType = type~! as? ConvertibleEnum.Type {
                return json.yy.enumArray(type: enumType)
            }
        }
    ...
    static func _dictionary(_ value: Any, _ type: Any.Type) -> DictionaryValue? {
        ...
        if let json = value as? [String: [String: Any]?],
            ...
            for (k, v) in json {
                guard let e = v?.yy.model(type: modelType) else { continue }
                modelDict[k] = e
            }
        ...
        if let json = value as? [String: [[String: Any]?]?],
            ...
            for (k, v) in json {
                guard let ma = v?.yy.modelArray(type: modelType) else { continue }
                modelArrayDict[k] = ma
            }
        ...
        if let enumType = subtype~! as? ConvertibleEnum.Type {
            ...
            for (k, v) in json {
                guard let ea = v?.yy.enumArray(type: enumType) else { continue }
                enumArrayDict[k] = ea
            }
    ...
    static func _JSONValue(from set: SetValue) -> Any {
        return _JSONValue(from: set.yy_array())
    }
...
postfix func ~! (_ value: Any?) -> Any? {
    return (value as OptionalValue).yy_value
}
