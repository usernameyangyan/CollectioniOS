...
public protocol Convertible {
    ...
    func yy_modelKey(from property: Property) -> ModelPropertyKey
    ...
    func yy_modelValue(from jsonValue: Any?,
    ...
    ///     - jsonValue: A jsonValue for the property
    ///     - property: A property of model
    func yy_modelType(from jsonValue: Any?,
    ...
    mutating func yy_willConvertToModel(from json: [String: Any])
    
    /// call when did finish converting from JSON to model
    ///
    /// - Parameters:
    ///     - json: A json did be converted to model
    mutating func yy_didConvertToModel(from json: [String: Any])
    ...
    /// - Parameters:
    ///     - property: A property of model
    func yy_JSONKey(from property: Property) -> JSONPropertyKey
...
public extension Convertible {
    ...
    func yy_willConvertToModel(from json: [String: Any]) {}
    func yy_didConvertToModel(from json: [String: Any]) {}
...
public extension Convertible {
    static var yy: ConvertibleYY<Self>.Type {
        get { return ConvertibleYY<Self>.self }
        set {}
    }
    var yy: ConvertibleYY<Self> {
        get { return ConvertibleYY(self) }
        set {}
    }
    
    /// mutable version of yy
    var yy_m: ConvertibleYY_M<Self> {
        mutating get { return ConvertibleYY_M(&self) }
        set {}
    }
}

public struct ConvertibleYY_M<T: Convertible> {
    ...
    public func convert(from jsonData: Data) {
        basePtr.pointee.yy_convert(from: jsonData)
    }
    
    /// JSONData -> Model
    public func convert(from jsonData: NSData) {
        basePtr.pointee.yy_convert(from: jsonData as Data)
    }
    
    /// JSONString -> Model
    public func convert(from jsonString: String) {
        basePtr.pointee.yy_convert(from: jsonString)
    }
    
    /// JSONString -> Model
    public func convert(from jsonString: NSString) {
        basePtr.pointee.yy_convert(from: jsonString as String)
    }
    
    /// JSONObject -> Model
    public func convert(from json: [String: Any]) {
        basePtr.pointee.yy_convert(from: json)
    }
    
    /// JSONObject -> Model
    public func convert(from json: [NSString: Any]) {
        basePtr.pointee.yy_convert(from: json as [String: Any])
    }
...
public struct ConvertibleYY<T: Convertible> {
    var base: T
    init(_ base: T) {
        self.base = base
    }
    ...
    public func JSONObject() -> [String: Any] {
        return base.yy_JSONObject()
    }
    
    /// Model -> JSONString
    public func JSONString(prettyPrinted: Bool = false) -> String {
        return base.yy_JSONString(prettyPrinted: prettyPrinted)
    }
}
...
extension Convertible {
    mutating func yy_convert(from jsonData: Data) {
        if let json = JSONSerialization.yy_JSON(jsonData, [String: Any].self) {
            yy_convert(from: json)
            return
        }
        Logger.error("Failed to get JSON from JSONData.")
    }
    
    mutating func yy_convert(from jsonString: String) {
        if let json = JSONSerialization.yy_JSON(jsonString, [String: Any].self) {
            yy_convert(from: json)
            return
        }
    ...
    mutating func yy_convert(from json: [String: Any]) {
        ...
        yy_willConvertToModel(from: json)
        
        // enumerate properties
        for property in properties {
            // key filter
            let key = mt.modelKey(from: property.name,
                                  yy_modelKey(from: property))
            
            // value filter
            guard let newValue = yy_modelValue(
                from: json.yy_value(for: key),
            ...
            if let modelType = yy_modelType(from: newValue, property),
        ...
        yy_didConvertToModel(from: json)
    }
    ...
    func _modelTypeValue(_ jsonValue: Any,
        ...
        if let json = jsonValue as? [Any] {
            let models = json.yy.modelArray(type: modelType)
            if !models.isEmpty {
        ...
        if let json = jsonValue as? [String: Any] {
            if let jsonDict = jsonValue as? [String: [String: Any]?] {
                ...
                for (k, v) in jsonDict {
                    guard let m = v?.yy.model(type: modelType) else { continue }
                    modelDict[k] = m
                }
                ...
                return json.yy.model(type: modelType)
            }
        }
        return jsonValue
    }
    ...
    func yy_JSONObject() -> [String: Any] {
        ...
        for property in properties {
            ...
            json[mt.JSONKey(from: property.name,
                            yy_JSONKey(from: property))] = v
        }
        
        yy_didConvertToJSON(json: json)
        
        return json
    }
    
    func yy_JSONString(prettyPrinted: Bool = false) -> String {
        if let str = JSONSerialization.yy_string(yy_JSONObject() as Any,
