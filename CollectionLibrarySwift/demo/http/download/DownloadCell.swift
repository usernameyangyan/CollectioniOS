...
class DownloadCell:YYTableViewCell{
    ...
    @objc func clikButton(tap:UITapGestureRecognizer) {
        ...
        case 0:
            ...
            let downloadStatus = YYHttpDowloadUtils.downloadStatus(url!)
            switch downloadStatus {
            case .complete:
            break     /// 完成
            case .downloading:  /// 暂停
                YYHttpDowloadUtils.downloadCancel(url!)
                self.updateCell()
            case .suspend:      /// 下载
        ...
        case 1:
            let item = self.item as! DownloadItem
            let url = item.downloadUrl
            YYHttpDowloadUtils.downloadDelete(url!) { [weak self] (result) in
                if result {
                    self?.updateCell()
                } else {
                    print("删除失败")
    ...
    func updateCell() {
        ...
        let progress = YYHttpDowloadUtils.downloadPercent(item.downloadUrl)
        progressView.progress = Float(progress)
        let status:DownloadStatus = YYHttpDowloadUtils.downloadStatus(item.downloadUrl)
