    ...
    */
    ...
    final func loadImage(url: URL,
                        placeholder: UIImage? = nil,
                         completion: ((_ success: Bool, _ error: Error?) -> Void)? = nil)
    {
        let cacheManager = YYImageCache.shared
                             ...
                             completion: ((_ success: Bool, _ error: Error?) -> Void)? = nil)
    {
        ...
        else if let cachedResponse = sharedURLCache.cachedResponse(for: request), let image = UIImage.process(data: cachedResponse.data), let creationTimestamp = cachedResponse.userInfo?["creationTimestamp"] as? CFTimeInterval, (Date.timeIntervalSinceReferenceDate - creationTimestamp) < Double(cacheManager.diskCacheMaxAge) {
            loadImage(image)
        ...
        // Either begin downloading the image or become an observer for an existing request.
            ...
            if cacheManager.isDownloadingFromURL(urlAbsoluteString) == false {
                ...
                let dataTask = cacheManager.session.dataTask(with: request) {
                    ...
                    guard let data = taskData, let response = taskResponse, let image =  UIImage.process(data: data), taskError == nil else {
                        DispatchQueue.main.async {
                    ...
                    DispatchQueue.main.async {
                        ...
                        let responseDataIsCacheable = cacheManager.diskCacheMaxAge > 0 &&
                            Double(data.count) <= Double(sharedURLCache.diskCapacity) &&
