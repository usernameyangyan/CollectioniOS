...
private var kYYRefreshHeaderKey: Void?
private var kYYRefreshFooterKey: Void?

public extension UIScrollView {
    
    /// Pull-to-refresh associated property
    var header: YYRefreshHeaderView? {
        get { return (objc_getAssociatedObject(self, &kYYRefreshHeaderKey) as? YYRefreshHeaderView) }
        set(newValue) { objc_setAssociatedObject(self, &kYYRefreshHeaderKey, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN) }
    }
    
    /// Infinitiy scroll associated property
    var footer: YYRefreshFooterView? {
        get { return (objc_getAssociatedObject(self, &kYYRefreshFooterKey) as? YYRefreshFooterView) }
        set(newValue) { objc_setAssociatedObject(self, &kYYRefreshFooterKey, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN) }
    }
}

public extension YY where Base: UIScrollView {
    /// Add pull-to-refresh
    
    @discardableResult
    func addPullToRefreshListener(animator: YYRefreshProtocol & YYRefreshAnimatorProtocol, handler: @escaping YYRefreshHandler,aboveView:UIView,childView:UIView) -> YYRefreshHeaderView {
        removeRefreshHeader()
        
        PositionSettingUtils.position(aboveView: aboveView, childView: childView, style: .fixed)

        let header = YYRefreshHeaderView(frame: CGRect.zero, handler: handler, animator: animator)
    ...
    @discardableResult
    func addLoadMoreListener(animator: YYRefreshProtocol & YYRefreshAnimatorProtocol, handler: @escaping YYRefreshHandler) -> YYRefreshFooterView {
        removeRefreshFooter()
        let footer = YYRefreshFooterView(frame: CGRect.zero, handler: handler, animator: animator)
    ...
    func stopPullToRefresh(ignoreDate: Bool = false, ignoreFooter: Bool = false) {
        ...
        if ignoreDate == false {
            if let key = self.base.header?.refreshIdentifier {
                YYRefreshDataManager.sharedManager.setDate(Date(), forKey: key)
            }
...
public extension UIScrollView /* Date Manager */ {
    ...
    var expired: Bool {
        get {
            if let key = self.header?.refreshIdentifier {
                return YYRefreshDataManager.sharedManager.isExpired(forKey: key)
            }
            return false
        }
    }
    ...
    var expiredTimeInterval: TimeInterval? {
        get {
            if let key = self.header?.refreshIdentifier {
                let interval = YYRefreshDataManager.sharedManager.expiredTimeInterval(forKey: key)
                return interval
            }
            return nil
        }
        set {
            if let key = self.header?.refreshIdentifier {
                YYRefreshDataManager.sharedManager.setExpiredTimeInterval(newValue, forKey: key)
            }
        }
    }
    ...
    var lastRefreshDate: Date? {
        get {
            if let key = self.header?.refreshIdentifier {
                return YYRefreshDataManager.sharedManager.date(forKey: key)
            }
            return nil
        }
    }
...
open class YYRefreshHeaderView: YYRefreshComponent {
    ...
    public convenience init(frame: CGRect, handler: @escaping YYRefreshHandler) {
        ...
        self.animator = YYDefaultRefreshHeaderAnimator.init()
    }
...
open class YYRefreshFooterView: YYRefreshComponent {
    fileprivate var scrollViewInsets: UIEdgeInsets = UIEdgeInsets.zero
    ...
    public convenience init(frame: CGRect, handler: @escaping YYRefreshHandler) {
        self.init(frame: frame)
        self.handler = handler
        self.animator = YYDefaultRefreshFooterAnimator.init()
    }
    
    /**
      In didMoveToSuperview, it will cache superview(UIScrollView)'s contentInset and update self's frame.
      It called YYRefreshComponent's didMoveToSuperview.
     */
    open override func didMoveToSuperview() {
        super.didMoveToSuperview()
        DispatchQueue.main.async {
